...
struct RegisterState {
    ...
    password_confirm_value: AttrValue,
...
pub fn register() -> Html {
    ...
    let oninput_password_confirm = {
        ...
        Callback::from(move |e: InputEvent| {
            ...
            let mut new_state = (*state).clone();
            new_state.password_confirm_value = input.value().into();
            state.set(new_state);
        })
    };
    ...
    let onclick_log_in = {
        ...
        Callback::from(move |_| {
            let mut new_state = (*state).clone();
            if state.password_value != state.password_confirm_value {
                new_state.error_message = Some("Passwords don't match".into());
                state.set(new_state);
                return;
            }
...

            {
                ...
                spawn_local(async move {
                    ...
                    new_state.error_message = match login_res {
                        ...
                        Ok(Err(ErrActions::UserExists)) => {
                            Some("A user with this username already exists.".into())
                        }
        ...
        <main class={ classes!("login-page", "floating-host") }>
            <div class={ classes!("login-menu", "floating-window") }>
                ...
                <div class={ classes!("login-menu-right") }>
                    ...
                    <TextBox input_type={ "password" } text={ "Confirm password" } classes={ classes!("full-width") } oninput={ oninput_password_confirm }  />
                    <div class={ classes!("login-menu-buttons") }>
                        <PushButton onclick={ onclick_log_in }>{ "Register" }</PushButton>
                    </div>
                </div>
                if state.loading {
